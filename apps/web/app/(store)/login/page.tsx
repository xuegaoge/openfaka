...
import { setTokens, authApi, withMockFallback, getApiErrorMessage, setTurnstileHeaders, userApi } from "@/services/api"
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // lin-cms 返回 { access_token, refresh_token, ... } 或 { token, user }
      if (result.access_token) {
        setTokens(result.access_token, result.refresh_token ?? result.access_token)
      ...
      if (result.user) {
        ...
        // 后端 login 没返回 user，主动获取用户信息
        try {
          const profile = await userApi.getProfile()
          setUser(profile)
        } catch {
          // ignore — token 已存储，下次加载会重试
        }
      }
