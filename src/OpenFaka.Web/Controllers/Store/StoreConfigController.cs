...
public class StoreConfigController : ControllerBase
{
    ...
    {
        // 尝试缓存
        var cached = await _cache.GetAsync(CACHE_KEY_SITE);
        ...
        await _cache.SetAsync(CACHE_KEY_SITE, json, CACHE_DURATION_SITE);
    ...
    {
        var cached = await _cache.GetAsync(CACHE_KEY_CHANNELS);
        if (!string.IsNullOrEmpty(cached))
            return Content(cached, "application/json");
        ...
        await _cache.SetAsync(CACHE_KEY_CHANNELS, json, CACHE_DURATION_CHANNELS);
        return Content(json, "application/json");
    }
